...
- **Supports Nonce:** Supports nonce attributes on script and style tags. Tag Helpers have been created for traditional CMS pages, must be applied in UI code for Headless solutions.
...
- **Supported Headers:** `Cross-Origin-Embedder-Policy`, `Cross-Origin-Opener-Policy`, `Cross-Origin-Resource-Policy`, `X-Content-Type-Options`, `X-XSS-Protection`, `X-Frame-Options`, `Referrer-Policy` and `Strict-Transport-Security` (HSTS)
